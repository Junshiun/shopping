...
export const CATEGORY_CHANGE = "change category";
...
export const CartReducer = (state, action) => {
  ...
  let total = (items) => {
    let results = items.reduce((total, currentValue) => {
  ...
  switch (action.type) {
    ...
    case FETCH_DATA:
      return {
        ...state,
        products: action.fetched.map((item) => {
          return {
            id: item.id,
            name: item.title,
            price: item.price,
    ...
    case ADD_TO_CART:
      ...
      return {
        ...
        cart: new_cart,
        status: { ...state.status, total: total(new_cart) },
      };

    case REMOVE_FROM_CART:
      new_cart = state.cart.filter((element) => {
    ...
    case CHANGE_QUANTITY_BLUR:
      new_cart = state.cart.map((item) => {
        if (item.id == action.product.id) {
          if (action.product.quantity < 1) item.quantity = 1;
    ...
    case ADD_QUANTITY:
      new_cart = state.cart.filter((item) =>
        item.id == action.product.id
          ? (item.quantity =
              item.quantity >= item.inStock ? item.inStock : item.quantity + 1)
          : item.quantity
      );
