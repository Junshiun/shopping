...
const Header = () => {
  ...
  useEffect(() => {
    ...
    if (cartbox == "show")
      document.getElementById("cart-box").style.height =
        height + (cart.length > 0 ? 48 : 16) + "px";
    else document.getElementById("cart-box").style.height = 0 + "px";
  });
    ...
    <header id="header">
        ...
        {locate === "/" || locate.includes("category") ? (
          <div onClick={handleClass} id="categories" ref={menu}>
            ...
            <div id="options" className="options hide">
              ...
              <a value="jewelery" onClick={handleCategories} className="option">
                jewelery
              </a>
              <a
      ...
      <div id="cart-container">
        <div className="cart-icon" onClick={updateCartbox}>
          <CgShoppingCart color="white" fontSize="25px"></CgShoppingCart>
          {cart.length > 0 ? (
        ...
        <div className="dropdown">
          <RiArrowDownSLine
            className={"arrow " + cartbox}
            onClick={updateCartbox}
            color="white"
            fontSize="25px"
          ></RiArrowDownSLine>
          {cart.length > 0 ? (
            <div id="cart-box" className={"cart-box " + cartbox}>
              <div id="measure-height">
                {cart.map((element) => {
                  return (
              ...
              <button className="checkout" onClick={() => history("/cart")}>
                go to cart
              </button>
            </div>
          ) : (
